...
public class ValidateVisitor implements Visitor {
	...
	public void visit(BooleanArgument argument, Option option) {
		...
		if (argument.isRequired() && optionValue == null) {
			this.errors.add("Argument for option '" + option.getNameWithPrefix() + "' is required.");
		}
		else if (optionValue != null) {
			if (!optionValue.equals("true") && !optionValue.equals("false")) {
				this.errors.add("Argument for option '" + option.getNameWithPrefix() + "' must be true/false.");
			}
		}	
	}
	...
	public void visit(EnumeratedArgument argument, Option option) {
		...
		if (argument.isRequired() && optionValue == null) {
			...
			if (!argument.getValues().contains(optionValue)) {
				this.errors.add("Argument for option '" + option.getNameWithPrefix() + "' must be " + String.join("/", argument.getValues()) + ".");
			}
		}	
	}
	...
	public void visit(IntegerArgument argument, Option option) {
		...
		if (argument.isRequired() && optionValue == null) {
			...
			if (!optionValue.matches("^[0-9]*$")) {
				this.errors.add("Argument for option '" + option.getNameWithPrefix() + "' must be integer.");
			}
			else if (Integer.parseInt(optionValue) < argument.getMinValue()) {
				this.errors.add("Argument for option '" + option.getNameWithPrefix() + "' must have minimum value " + argument.getMinValue() + ".");
			}
			else if (Integer.parseInt(optionValue) > argument.getMaxValue()) {
				this.errors.add("Argument for option '" + option.getNameWithPrefix() + "' must have maximum value " + argument.getMaxValue() + ".");
			}
		}
	}
	...
	public void visit(StringArgument argument, Option option) {
		...
		if (argument.isRequired() && optionValue == null) {
			...
			if (optionValue.length() < argument.getMinLength()) {
				this.errors.add("Argument for option '" + option.getNameWithPrefix() + "' must have minimum length of " + argument.getMinLength() + ".");
			}
			else if (optionValue.length() > argument.getMaxLength()) {
				this.errors.add("Argument for option '" + option.getNameWithPrefix() + "' must have maximum length of " + argument.getMaxLength() + ".");
			}
		}
	}
	...
	public void visit(Option option) {
		...
		if (isOptionUsed) {
			...
			this.errors.add("Option '" + option.getNameWithPrefix() + "' is required.");
		}
	}
